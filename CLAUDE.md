...
- **方向の調整（カーソル＝矢印を一致）**: `zip_gesture` の override にはカーソル base と**同じ `zip_xy_transform` フラグ**を前段に置いてあり、矢印の向きは**常にカーソルの向きと一致**する。C 側の方向マッピングは画面座標規約（post-transform +X=右=RIGHT、+Y=下=DOWN）で固定。向きを変えたいときは `src/trackball_gesture.c` ではなく、`&pointing_listener` の **3か所の `zip_xy_transform` フラグ**（base / `fast_mouse` / `gesture` override）を**同一に**変更する：X を反転するなら `INPUT_TRANSFORM_X_INVERT`、Y なら `INPUT_TRANSFORM_Y_INVERT`。現状は `0`（反転なし＝ボール追従）。元の `INVERT|INVERT` はカーソルがボールと逆だったため変更した。なお scroll レイヤー（6/7）の transform は別管理で未変更。
