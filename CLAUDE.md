...
- **仮想ポジション**: `boards/shields/torabo_tsuki_lp/torabo_tsuki_lp.dtsi` の `size_l_transform` 末尾に未配線の `RC()` を4つ追加して確保（キーマップ位置 **66=UP, 67=DOWN, 68=LEFT, 69=RIGHT**）。物理マトリクスでは配線されていないため、スキャンからは発火せずジェスチャーハンドラからのみ発火する。これに伴い**全レイヤーのバインド数は 66→70** に揃えてある（ZMK は最大レイアウト=L にキーマップ長を合わせる仕様）。
