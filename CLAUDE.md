...
- **仕組み**: `src/trackball_gesture.c` が `INPUT_CALLBACK_DEFINE(NULL, ...)`（central限定、NULL=全デバイス）でトラックボールの生X/Y変位を購読・蓄積し、ジェスチャーレイヤーが有効な間だけ、閾値超過で方向を確定して**キーポジションを press/release** する。アクションの割り当ては通常のキーマップ側（そのポジションのバインド）で行うため、C の再コンパイル不要。
...
- **1モーション1アクション**: 連続した1回のボール移動につき1回だけ発火し、ボールが `GESTURE_IDLE_REARM_MS` の間アイドルになると再武装する（転がし続けても連射しない）。発火は raise をシステムワークキューに退避して行う（入力スレッドから直接 raise しない）。
- **カーソル抑制は現状なし**: 当初 `gesture_suppress`（`zip_xy_scaler 0`）で Layer 9 のカーソルを止めていたが、**入力コールバックは同一イベント実体を順に受け取り、リスナー（カーソル処理）が検出コールバックより先に走るため、scaler 0 が検出側の読み取り値もゼロ化して発火しなくなる**問題が判明したため削除した。よってジェスチャー中もカーソルは動く。抑制を入れ直すには、検出をリスナーより前で行う（入力プロセッサ化など）別アプローチが必要。
- **調整パラメータ**（`src/trackball_gesture.c` 冒頭の `#define`）: `GESTURE_THRESHOLD`（発火に必要な変位量）, `GESTURE_IDLE_REARM_MS`（このms間アイドルで再武装＝次の1アクションが可能になるまでの静止時間）, `GESTURE_RELEASE_DELAY_MS`（press→release 間隔）, `gesture_layers[]`（ジェスチャー有効レイヤーの配列）。方向が逆に感じる場合は `GESTURE_POS_*` の対応を入れ替える。
