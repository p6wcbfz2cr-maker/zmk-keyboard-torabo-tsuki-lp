...
* `/` キーをホールドしている間だけクリック・高速マウスにアクセスでき、離せば即座に通常のタイピングへ戻るため、トラックボール操作直後の反射的なタイピングがホームローに奪われなくなるはず。
* `/` の単発タップは従来どおり `/` を出力し、Layer 5 に誤って入らないことを実機で確認する。
* ブランチでの試験運用のため、`master` にはまだマージしていない。使用感次第で起動キーの位置調整や、自動遷移の部分的な復活を検討する可能性がある。
