...
| `max-speed` | 4800 counts/s | 取り込む正規化後速度の上限。前回の6000から下げ、慣性開始直後の速さを抑える |
| `friction-permille` | 960 | 1 tick後に残す速度（1000分率）。前回の940から上げ、滑走距離を増やす |
...
開始速度付近では約0.69秒、上限速度付近では約0.96秒で停止する想定。最高速は前回より20%低い一方、慣性中の総移動量は増える。
