...
| プロパティ | 初期値 | 意味 |
|---|---:|---|
| `start-speed` | 800 counts/s | 慣性を開始する正規化後の最低速度。前回の1200から下げ、より軽いフリックでも開始する |
| `stop-speed` | 35 counts/s | 慣性を終了する正規化後の速度 |
| `max-speed` | 3600 counts/s | 取り込む正規化後速度の上限。前回の4800から下げ、慣性開始直後の速さを抑える |
| `friction-permille` | 970 | 1 tick後に残す速度（1000分率）。前回の960から上げ、より緩やかに減速させる |
...
開始速度付近では約0.82秒、上限速度付近では約1.22秒で停止する想定。最高速は前回より25%低く、より緩やかに減速する。
