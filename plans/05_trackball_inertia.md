...
| プロパティ | 初期値 | 意味 |
|---|---:|---|
| `start-speed` | 200 counts/s | 慣性を開始する正規化後の最低速度。より早い段階で慣性を開始する |
| `stop-speed` | 1 count/s | 慣性を終了する正規化後の速度。低速域の滑走を長く残す |
| `max-speed` | 1500 counts/s | 取り込む正規化後速度の上限。慣性開始直後の速さを大きく抑える |
...
開始速度付近では約2.1秒、上限速度付近では約2.9秒で停止する想定。低い開始速度で早めに発動しつつ、最高速を大きく抑え、低速域の滑走を長く残す。
