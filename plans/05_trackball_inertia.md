...
| プロパティ | 初期値 | 意味 |
|---|---:|---|
| `start-speed` | 500 counts/s | 慣性を開始する正規化後の最低速度。より早い段階で慣性を開始する |
| `stop-speed` | 35 counts/s | 慣性を終了する正規化後の速度 |
| `max-speed` | 3200 counts/s | 取り込む正規化後速度の上限。慣性開始直後の速さをさらに抑える |
...
開始速度付近では約0.41秒、上限速度付近では約0.70秒で停止する想定。低い開始速度で早めに発動しつつ、最高速は抑える。
