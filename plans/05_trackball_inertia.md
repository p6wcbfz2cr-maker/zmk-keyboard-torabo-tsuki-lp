...
| プロパティ | 初期値 | 意味 |
|---|---:|---|
| `start-speed` | 400 counts/s | 慣性を開始する正規化後の最低速度。より早い段階で慣性を開始する |
| `stop-speed` | 10 counts/s | 慣性を終了する正規化後の速度。低速域の滑走を残す |
| `max-speed` | 2500 counts/s | 取り込む正規化後速度の上限。慣性開始直後の速さをさらに抑える |
| `friction-permille` | 950 | 1 tick後に残す速度（1000分率）。緩やかさと素早い収束の中間にする |
...
開始速度付近では約0.58秒、上限速度付近では約0.86秒で停止する想定。低い開始速度で早めに発動しつつ、最高速を抑え、低速域の滑走を残す。
