...
現在は `scroll_win`（Layer 7、Windowsモード）にのみ適用する。`scroll_mac`（Layer 6）は慣性導入前の設定（`zip_xy_transform (INPUT_TRANSFORM_X_INVERT)` のみ、`zip_scroll_inertia` 参照なし）に戻してあり、慣性は適用しない。
...
既存のスクロール倍率が異なる経路でも同じ閾値を使えるよう、プロセッサの2セル引数で処理済み速度を共通の基準へ戻してから発火判定する（現状はWindows経路のみが対象）。
...
Mac経路（`scroll_mac`, Layer 6）は慣性導入前の設定に戻したため、`zip_scroll_inertia` を参照しない。方向は `INPUT_TRANSFORM_X_INVERT` のみ（慣性導入時に一時的に加えていた `INPUT_TRANSFORM_Y_INVERT` も併せて取り消した）。
...
- `tail-speed` 以下ではソフト終端フェーズへ入り、`friction-permille` から `tail-friction-permille` までを速度に応じて連続的に補間し、速度と端数を同時に減衰させる。
- `tick-ms` ごとに `friction-permille / 1000` を速度へ掛け、`stop-speed` 未満で終了する。
- 停止閾値を下回ったtickでは、ソフト終端で減衰済みの端数を送信せず破棄する。停止直前の単発スクロールと急停止感を抑えるためである。
...
| `tail-speed` | 100 counts/s | これ以下でソフト終端フェーズへ移る速度 |
| `friction-permille` | 950 | 1 tick後に残す速度（1000分率）。通常フェーズを早めに減速させる |
| `tail-friction-permille` | 985 | 停止速度付近で速度と端数へ適用する減衰率。通常値から連続補間する |
...
開始速度付近では約1.2秒、上限速度付近では約2.8秒で停止する想定。低い開始速度で早めに発動しつつ、最高速を抑え、低速域はソフト終端で滑らかに減速する。
